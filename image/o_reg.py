...
def train(model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        ortho_loss = orthogonal_regularization(model.first_two_linear_combined_weight())
        loss = F.cross_entropy(logits, target) + ortho_loss
        loss.backward()
        optimizer.step()
...
def train_with_teacher(model, device, train_loader, optimizer, criterion, epoch, teacher):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        ortho_loss = orthogonal_regularization(model.first_two_linear_combined_weight())
        loss = criterion(logits, target) + ortho_loss
        loss.backward()
        optimizer.step()
...
def main():
    ...
    loss_log_path = "rankgraphs/normperserving_losses.csv"
